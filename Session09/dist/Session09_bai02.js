...
class Audience {
    ...
    phone;
    id;
    constructor(name, email, phone, id = nextAudienceId++) {
        this.name = name;
        this.email = email;
        this.phone = phone;
        this.id = id;
...
class Movie {
    ...
    isShowing;
    id;
    constructor(title, genre, ticketPrice, isShowing = true, id = nextMovieId++) {
        this.title = title;
        this.genre = genre;
...
class TicketBooking {
    ...
    bookingId;
    totalPrice;
    constructor(audience, movie, quantity, bookingId = nextBookingId++) {
...
class Cinema {
    ...
    audiences = [];
    bookings = [];
    addMovie(m) { this.movies.push(m); }
    addAudience(n, e, p) {
        let a = new Audience(n, e, p);
        this.audiences.push(a);
        return a;
    }
    bookTickets(aid, mid, qty) {
    ...
    getMovieSpecialOffers(mid) {
        let m = this.movies.find(x => x.id === mid);
        return m ? m.getSpecialOffers().join("\n") : "";
    }
}
function seed(cinema) {
...
Chọn:`);
        ...
        switch (ch) {
            case "1":
                ...
                if (name && email && phone)
                    alert("Đã thêm: " + c.addAudience(name, email, phone).getDetails());
                break;
            case "2":
                ...
                if (t && p > 0) {
                    if (g === "action")
                        c.addMovie(new ActionMovie(t, "Action", p));
                    else if (g === "comedy")
                        c.addMovie(new ComedyMovie(t, "Comedy", p));
                    else if (g === "animation")
                        c.addMovie(new AnimationMovie(t, "Animation", p));
                    alert("Đã thêm phim");
                }
                break;
            case "3":
            ...
            case "9":
                ...
                if (type === "1")
                    alert(c.movies.find(m => m.id === id)?.getBasicInfo() || "Không tìm thấy");
                if (type === "2")
                    alert(c.audiences.find(a => a.id === id)?.getDetails() || "Không tìm thấy");
                if (type === "3")
                    alert(c.bookings.find(b => b.bookingId === id)?.getDetails() || "Không tìm thấy");
                break;
            case "10":
